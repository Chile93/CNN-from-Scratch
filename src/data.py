...
def get_data_loaders(
    ...
    data_transforms = {
        ...
        "valid": transforms.Compose(
            [
                transforms.Resize((256, 256)),  # Resize to 256x256
                transforms.CenterCrop(224),  # Center crop to 224x224 for validation
                transforms.ToTensor(),  # Convert to Tensor
                transforms.Normalize(mean=mean, std=std),  # Normalize
            ]
        ),
        "test": transforms.Compose(
            [
                transforms.Resize((256, 256)),  # Resize to 256x256
                transforms.CenterCrop(224),  # Center crop to 224x224 for test
                transforms.ToTensor(),  # Convert to Tensor
                transforms.Normalize(mean=mean, std=std),  # Normalize
            ]
        ),
    ...
    # Define the test sampler
    # test_sampler = None if limit > 0 else torch.utils.data.SubsetRandomSampler(indices)
    if limit > 0:
